#include "icmp_master.h"
#include "icmp_slave.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void printHelp()
{
  printf("-m, --master                        run the binary as a master "
         "(server), it's the default value\n");
  printf("-s, --slave <Master IP>             run the binary as a slave "
         "(client)\n");
    ...
    else if (strcmp(arg, "-s") == 0 || strcmp(arg, "--slave") == 0)
    {
      ...
      if (argc >= 3)
      {
            ...
            && (strcmp(argv[3], "-f") == 0 || strcmp(argv[3], "--file") == 0))
        {
          ...
          int result = modeSlave(serverIP, fileName);
          if (result == 0)
          {
